...
func composeRouterDeps(db *pgxpool.Pool, opt Options, pub publicServices, adm adminServices, auth ports.AuthService) httpapi.RouterDeps {
	return httpapi.RouterDeps{
		...
		Security: httpapi.SecurityHeadersConfig{
			HSTS:                  opt.SecurityHSTS,
			HSTSMaxAge:            opt.SecurityHSTSMaxAge,
			ContentSecurityPolicy: opt.SecurityCSP,
			FrameAncestors:        opt.SecurityFrameAncestors,
		},
	}
}
